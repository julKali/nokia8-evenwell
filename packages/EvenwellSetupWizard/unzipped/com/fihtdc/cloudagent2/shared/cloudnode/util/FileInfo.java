...
public class FileInfo {
    ...
    public FileInfo(String path, boolean isDirectory, long parentId, long size,
            long modifyTime, long createTime) {
        data.put(FileContract.File.DATA, path);
        data.put(FileContract.File.PARENT, parentId);
        data.put(FileContract.File.SIZE, size);
        data.put(FileContract.File.DATE_MODIFIED, modifyTime);
        data.put(FileContract.File.DATE_ADDED, createTime);
        data.put(FileContract.File.SERVER_PATH, path);
        this.isDirectory = isDirectory;
        if (path != null) {
            File file = new File(path);
            File parentFile = file.getParentFile();
            String mimeType = getMimeType(path);
            data.put(FileContract.File.DISPLAY_NAME, file.getName());
            data.put(FileContract.File.TITLE, MediaFile.getFileTitle(path));
            data.put(FileContract.File.NAME, FileListUtil.cutSlashInEnd(file.getName()));
            data.put(FileContract.File.BUCKET_ID, parentFile.toString().toLowerCase().hashCode());
            data.put(FileContract.File.BUCKET_DISPLAY_NAME, parentFile.getName());
            data.put(FileContract.File.MIME_TYPE, mimeType);
            data.put(FileContract.File.MEDIA_TYPE, checkMediaType(mimeType));
            data.put(FileContract.File.FORMAT, getFormatCode(path, mimeType, isDirectory));
            data.put(FileContract.File.IS_MUSIC, (mimeType != null && mimeType.startsWith("audio/")));
        } else {
            Log.e(TAG, "FileInfo() path is null.");
        }

        if (size < 0) {
            Log.w(TAG, "FileInfo() size is less than 0.");
        }
        if (modifyTime < 0) {
            Log.w(TAG, "FileInfo() modifyTime is less or equal 0.");
        }
        if (createTime < 0) {
            Log.w(TAG, "FileInfo() createTime is less or equal 0.");
        }
    }
    @Override
    public String toString() {
        return "[FileInfo]: "+data.toString();
    }
    ...
    public static int getFormatCode(String path, String mimeType, boolean isDirectory) {
        if (isDirectory)
            return MtpConstants.FORMAT_ASSOCIATION;

        return MediaFile.getFormatCode(path, mimeType);
    }

    public static int checkMediaType(String mimeType) {
        if (mimeType == null)
            return FileContract.MEDIA_TYPE_NONE;
        if (mimeType.startsWith("audio"))
            return FileContract.MEDIA_TYPE_AUDIO;
        else if (mimeType.startsWith("image"))
            return FileContract.MEDIA_TYPE_IMAGE;
        else if (mimeType.startsWith("video"))
            return FileContract.MEDIA_TYPE_VIDEO;
        else
            return FileContract.MEDIA_TYPE_NONE;
    }

    public static String getMimeType(String file) {
        String sub_name = file.substring(file.lastIndexOf('.') + 1).toLowerCase();
        String mime_type = sMimeTypeMap.get(sub_name);
        return mime_type;
    }

    private static void add(String value, String key) {
        if (sMimeTypeMap != null) {
            if (sMimeTypeMap.get(key) == null)
                sMimeTypeMap.put(key, value);
        }
    }

    static HashMap<String, String> sMimeTypeMap;
    ...
    static {
        ...
        if (sMimeTypeMap == null) {
            ...
            add("text/iuls", "uls");
            add("text/mathml", "mml");
            // add ".txt" first so it will be the default for
            ...
            /* add for FIH NOTE --20130328 */
            add("application/fih-note", "note");
            /* add for .7z support --20130516 */
